...
/*	Loader::import('weixinpay.SdkRuntimeException.SdkRuntimeException');
	Loader::import('weixinpay.WxPayPubConfig.WxPayConf_pub');*/
...
class JsApi_pub extends Common_util_pub
{
	...
	var $info;//用户的网页授权登录的信息 包括access_token和openid
	var $parameters;//jsapi参数，格式为json
	var $prepay_id;//使用统一支付接口得到的预支付id
	var $curl_timeout;//curl超时时间
	var $appid;//curl超时时间
	var $appsecret;//curl超时时间
	...
	/**
	 * 	作用：生成可以获得code的url 弹出授权页面
	 */
	function CodeUserinfo1($redirectUrl)
	{
		$urlObj["appid"] = WxPayConf_pub::APPID;
		$urlObj["redirect_uri"] = "$redirectUrl";
		$urlObj["response_type"] = "code";
	...
	/**
	 * 	作用：通过curl向微信提交code，以获取openid和token
	 */
	function getInfo()
	{
		$url = $this->createOauthUrlForOpenid();
