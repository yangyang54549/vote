<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:60:"E:\GitHub\vote\public/../application/web\view\gold\info.html";i:1519640646;}*/ ?>
<!doctype html>
<html lang="en">
  ...
  中国建设银行  户名：王 君   卡号：4367 4200 1350 0901 552-->
							...
							<!--<div class="mui-card mui-row" style="padding: 10px;">
								...
								<div class="mui-col-sm-6 mui-col-xs-6" id="ke">
									联系客服
								</div>
				...
				if(tt > 32) {
					...
					if(ons > 0) {
						...
						ons = ons - 1;
						 ...
						 mui.toast('已使用完每天三次的点赞功能',{ duration:'long', type:'div' })
					}
				}
			});*/
			mui(".mui-row").on("tap", ".mui-row #gm", function() {
				//当前对象直接就是--->this
				var th = this;
				/*mui.toast('已使用完每天三次的点赞功能',{ duration:'long', type:'div' }) */
				mui.confirm("确认购买？", '提示', ['取消', '确认'], function(e) {

					if(e.index == 0) {
						//点击取消	this.text("123123");

					} else {
window.location.href = "dingdan1.html";
						//确认

					}
				}, 'div');
				//mui.alert("确认购买？",'提示',['确认'],null,'div')
				//mui.prompt(" message", "placeholder", "title", ['取消','确认'], null,'div')

			});
			mui(".mui-row").on("tap", ".mui-row #ke", function() {
				window.location.href = "tel:13613820359 ";

				/*mui.prompt("确认购买？", '提示', "表单", function(e) {
					if(e.index == 0) {
						//点击取消	this.text("123123");
					} else {
						//确认
