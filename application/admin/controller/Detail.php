...
class Detail extends Controller
{
    ...
    public function withdraw()
    {
        ...
        // 列表过滤器，生成查询Map对象
        $map = $this->search($model, [$this->fieldIsDelete => $this::$isdelete,'or'=>2]);
    ...
    public function wedit()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            //如果审核未通过,就返回提现金额和手续费
            if ($data['status']==2) {
                $jia = $data['integral'] + $data['charge']*100;
                Db::table('tp_user')->where('id',$data['user_id'])->setInc('integral',$jia);
            }
