...
class Wxpay extends Yang
{
    ...
    public function getinfo(){
        if ( strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') !== false ) {
            ...
            if (isset($result)) {
                ...
                //非必填参数，商户可根据实际情况选填
                //return $openid.'---'.$result.'---'.$NOTIFY_URL;
    ...
    public function weixinjsapnotify() {
        ...
        if($notify->checkSign() == TRUE){
            if ($notify->data["return_code"] == "FAIL") {
                ...
                /*更新订单状态这里写数据库的操作*/
                $info = Detail::where(['id'=>$out_trade_no])->find();
                if($info['status'] == 1){
                    //已经修改订单状态
                    return 'SUCCESS';
                }else{
                    ...
                    try{
                        //修改数据库 增加余额 修改状态
                        Detail::where(['id'=>$out_trade_no])->update(['status'=>1]);
                        User::where(['openid'=>$openid])->setInc('integral',$total_fee);
                        // 提交事务
                        Db::commit();
    ...
    public function activate(){
        if ( strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') !== false ) {
            ...
            if (isset($result)) {
                ...
                //设置统一支付接口参数
                //设置必填参数
                $NOTIFY_URL=URLL.url('wxpay/weixinactivate');
                $unifiedOrder->setParameter("openid",$openid);//openid
                $unifiedOrder->setParameter("body",'充值');//商品描述
                $unifiedOrder->setParameter("out_trade_no",$result);//商户订单号
                $unifiedOrder->setParameter("total_fee",$money);//总金额
     ...
     * @return [type] [description]
     */
    public function weixinactivate() {
        Db::table('tp_jilu')->insert(['suiyi'=>555]);
        $name = 'paylog.txt';
        //使用通用通知接口
        Loader::import('Weixinpay.WxPayPubHelper', EXTEND_PATH);
        $notify = new \Notify_pub();
